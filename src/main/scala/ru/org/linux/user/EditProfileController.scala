...
class EditProfileController(
  ...
  def edit(
    ...
    AuthorizedOnly { implicit currentUser =>
      ...
      if newPassword.exists(_.equalsIgnoreCase(nick)) then
        errors.reject(null, "пароль не может совпадать с логином")

      val mail = Option(form.getEmail)
        .filter(_.nonEmpty)
        .flatMap { mail =>
          try
      ...
      val newEmail = mail.flatMap { mail =>
        if user.email != null && user.email == mail.getAddress.toLowerCase then
          None
        else
          if userDao.getByEmail(mail.getAddress.toLowerCase, false) != 0 then
